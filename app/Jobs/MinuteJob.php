...
class MinuteJob extends Job implements ShouldQueue {
    ...
    public function handle() {
        //\Log::info('No MINUTE job for mysurvey');
    }
}
...
//                     \Log::info('SurveyPublished today: User'.$user->name.' id '.$survey->id );
...
//                     \Log::info('SurveyPublished + 7 days: User'.$user->name.' id '.$survey->id );
...
//                 $date14 = $date->modify('+14 day');
    
//                 if ($survey_results == NULL && $date14 == new  \DateTime('today')) { 
//                     \Log::info('SurveyPublished + 14 days: User'.$user->name.' id '.$survey->id );
